...
export class IacStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const alternativeDomain = process.env.ALTERNATIVE_DOMAIN_NAME || 'reservation-dev.devmaua.com'
    const hostedZoneIdValue = process.env.HOSTED_ZONE_ID || 'Z1UJRXOUMOOFQ8'
    const projectName = process.env.PROJECT_NAME || 'MauaReservationFront'

    const s3Bucket = new s3.Bucket(this, 'MauaReservationFrontBucket' + stage, {
      versioned: true,
    ...
    const oac = new cloudfront.CfnOriginAccessControl(this, 'AOC', {
      originAccessControlConfig: {
        name: 'Maua Reservation Front Bucket OAC ' + stage,
    ...
    if (stage === 'prod' || stage === 'homolog' || stage === 'dev') {
      viewerCertificate = cloudfront.ViewerCertificate.fromAcmCertificate(
        Certificate.fromCertificateArn(this, 'MauaReservationFrontCertificate-' + stage, acmCertificateArn),
        {
    ...
    const cloudFrontWebDistribution = new cloudfront.CloudFrontWebDistribution(
      ...
      'CDN',
      {
        comment: 'Maua Reservation Front Distribution ' + stage,
        originConfigs: [
          {
            s3OriginSource: {
    ...
    if (stage === 'prod' || stage === 'homolog' || stage === 'dev'){
      const zone = route53.HostedZone.fromHostedZoneAttributes(
        this,
        'MauaReservationFrontHostedZone-' + stage,
        {
        ...
        new route53.ARecord(this, 'MauaReservationFrontAliasRecord-' + stage, {
          zone: zone,
      ...
      new cdk.CfnOutput(this, 'MauaReservationFrontBucketName-' + stage, {
        value: s3Bucket.bucketName
      })

      new cdk.CfnOutput(this, 'MauaReservationFrontDistributionId-' + stage, {
        value: cloudFrontWebDistribution.distributionId
      })

      new cdk.CfnOutput(this, 'MauaReservationFrontDistributionDomainName-' + stage, {
